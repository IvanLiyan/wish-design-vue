  ...
  <div :class="classes">
    <div :class="[prefixCls+ '-list']" ref="hours">
      <ul :class="[prefixCls + '-ul']">
        <li :class="getCellCls(item)" v-for="(item, i) in hoursList"
        :key="i" v-show="!item.hide"
        @click="handleClick('hours', item)">{{ formatTime(item.text) }}</li>
      </ul>
    </div>
    <div :class="[prefixCls+ '-list']" ref="minutes" v-show="showMinutes">
      <ul :class="[prefixCls + '-ul']">
        <li :class="getCellCls(item)" v-for="(item, i) in minutesList"
          :key="i" v-show="!item.hide"
    ...
    <div :class="[prefixCls+ '-list']" v-show="showSeconds" ref="seconds">
      <ul :class="[prefixCls + '-ul']">
        <li :class="getCellCls(item)" v-for="(item, i) in secondsList"
        :key="i" v-show="!item.hide"
...
import { deepCopy, firstUpperCase, scrollTop } from '@/utils/util';
...
function getFirstEnableCell (cells) {
  return cells.find((cell) => !cell.disabled);
}
...
export default {
  ...
  computed: {
    ...
    secondsList () {
      ...
      for (let i = 0; i < 60; i += step) {
        ...
        if (this.disabledSeconds.length &&
  ...
  watch: {
    hours (val) {
      ...
      this.scroll('hours', this.hoursList.findIndex(obj => {
    ...
    minutes (val) {
      if (!this.compiled) return;
      this.scroll('minutes',
        this.minutesList.findIndex(obj => parseInt(obj.text) === val));
    },
    seconds (val) {
      if (!this.compiled) return;
      this.scroll('seconds',
        this.secondsList.findIndex(obj => parseInt(obj.text) === val));
    },
    focusedTime (updated, old) {
      timeParts.forEach((part, i) => {
        if (updated[i] === old[i] || typeof updated[i] === 'undefined') return;
        const valueIndex = this[`${part}List`].findIndex((obj) => {
  ...
  mounted () {
    this.$nextTick(() => { this.compiled = true; this.updateScroll(); });
  },
  methods: {
    getCellCls (cell) {
      const { prefixCls } = this;
    ...
    handleClick (type, cell) {
      ...
      const data = { };
      const items = [
        ['hours', this.hours, this.hoursList],
        ['minutes', this.minutes, this.minutesList],
        ['seconds', this.seconds, this.secondsList],
      ];
      items.forEach((item) => {
        const [t, value, list] = item;
        if (isNaN(value)) {
          const i = getFirstEnableCell(list);
          const v = i ? i.text : value;
          if (!isNaN(v)) {
            data[t] = v;
          }
        }
    ...
    updateScroll () {
      this.$nextTick(() => {
        timeParts.forEach(type => {
          this.$refs[type].scrollTop = 30 * this[`${type}List`]
            .findIndex(obj => parseInt(obj.text) === this[type]);
        });
      });
    },
    formatTime (text) {
