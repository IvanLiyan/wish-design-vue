<template>
  <wt-form :model="formCustom" ref="formCustom" :rules="ruleCustom">
    <wt-form-item label="主机名称" prop="hostname">
      <wt-input type="text" v-model="formCustom.hostname"
        style="width: 260px;" />
    </wt-form-item>
    ...
    <wt-form-item label="购买台数" prop="number" style="width: 260px;">
      <wt-select type="text" v-model="formCustom.number" style="width: 100px;">
        <wt-option :value="1" label="1台" />
        <wt-option :value="2" label="2台" />
        <wt-option :value="3" label="3台" />
        <wt-option :value="4" label="4台" />
      </wt-select>
    </wt-form-item>
    <wt-form-item label="硬盘容量" prop="size">
      <wt-slider v-model="formCustom.size" style="width: 350px;" />
    </wt-form-item>
    <wt-form-item label="主机编号" prop="serilize">
      <div
    ...
    <wt-form-item label="访问地址" prop="url">
      <wt-input v-model="formCustom.url" style="width: 400px;">
        <template slot="prepend">http://</template>
        <template slot="append">
          <wt-select v-model="formCustom.appendValue" style="width: 90px;">
            <wt-option value=".com" label=".com" />
            <wt-option value=".cn" label=".cn" />
          </wt-select>
        </template>
      </wt-input>
    </wt-form-item>
    <wt-form-item label="自动续费" prop="renewal">
      <wt-checkbox v-model="formCustom.renewal">到期后自动续费</wt-checkbox>
    </wt-form-item>
    <wt-form-item label="标签" prop="tags">
      <wt-select v-model="formCustom.tags" placeholder="请选择" multiple
        style="width: 260px;">
        <wt-option
          v-for="item in list2"
          :key="item.value"
    ...
    <wt-form-item>
      ...
      <wt-button @click="handleReset">取消</wt-button>
    </wt-form-item>
  </wt-form>
</template>
