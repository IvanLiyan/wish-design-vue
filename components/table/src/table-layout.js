...
class TableLayout {
  ...
  updateScrollY () {
    ...
    if (this.table.$el && bodyWrapper) {
      const body = bodyWrapper.querySelector(`.${this.table.prefix}-body`);
  ...
  updateElsHeight () {
    ...
    if (this.height !== null && (!isNaN(this.height) ||
      ...
      const footerHeight = this.footerHeight = footerWrapper
        ? footerWrapper.offsetHeight : 0;
      this.bodyHeight = tableHeight - headerHeight - footerHeight +
        (footerWrapper ? 1 : 0);
    }
  ...
  updateColumnsWidth () {
    ...
    if (flexColumns.length > 0 && fit) {
      ...
      if (bodyMinWidth <= bodyWidth - scrollYWidth) { // DON'T HAVE SCROLL BAR
        ...
        if (flexColumns.length === 1) {
          flexColumns[0].realWidth = (flexColumns[0].minWidth || 80) +
            totalFlexWidth;
        } else {
          const allColumnsWidth = flexColumns.reduce(
            (prev, column) => prev + (column.minWidth || 80), 0,
          );
    ...
    const fixedColumns = this.table.fixedColumns;
    ...
    const rightFixedColumns = this.table.rightFixedColumns;
